<script setup>
    ...
    const trushDevice = () => {
        useAppSpinnerStore().show("verification");
        authConsumer
            .trustDevice(user)
            .then((responce) => {
                useAppSpinnerStore().hide();
                useAppAlertStore().show("verification" , responce.message);
            })
            .catch((error) => {
                console.error(error);
            });
...
<template>
    <section>
        ...
        <FormeVue >
            <template #alert>
                <Alert v-if="useAppAlertStore().getStatus == 'verification'" />
            </template>
            <template #spinner>
                <Spinner v-if="useAppSpinnerStore().getStatus == 'verification'" :trenspBackg="true" />
            </template>
